...
void right_hand(void){
	set_straight(142.0, nomal_run.accel, nomal_run.vel_search, 0.0, 0.0);
	wait_straight();
	while(1){
		if(SEN_L.now<SEN_L.threshold){
			set_straight(90.0, nomal_run.accel, nomal_run.vel_search, 0.0, 0.0);
			wait_straight();
			...
			set_straight(180.0, nomal_run.accel, nomal_run.vel_search, 0.0, 0.0);
						wait_straight();
